...
![limit语句加锁](./images/limit语句加锁.jpg)

执行过程：

T1：session A开启事务，并删除c=10的行，并限制2条。

T2：session B插入(13,13,13)，执行成功。

分析：

1. session A在遍历的时候，先访问到c=10的记录，根据原则一，这里加的是(c=5,id=5)到(c=10,id=10)的`next-key lock`。

2. 继续向右遍历，遍历到(c=10,id=30)这一行，一共两行，此时就结束到了，所以会增加(c=10,id=10)到(c=10,id=30)的`next-key lock`。

![limit语句加锁](./images/limit语句加锁.png)

所以session B插入(13,13,13)是成功的。

**案例八：死锁的例子**

![死锁的例子](./images/死锁例子.jpg)

执行过程：

T1：session A开启事务，查询c=10的行并加上共享读锁。

T2：session B更新c=10的行，被阻塞。

T3：session C插入一行(8,8,8)成功，同时死锁检测杀死session B。

分析：

1. session A查询c=10，需要先遍历到c=10，给(5,10]加上`next-key lock`。

2. 再向右遍历，再找到第一个不满足c=10，给(10,15)加上间隙锁。

3. session B更新c=10，也要在索引c上加`next-key lock`(5,10]，进入锁等待（间隙锁是成功的，但10上的行锁需要等待）。

4. session A插入(8,8,8)时，发现session B有(5,10)的间隙锁，进入锁等待，触发死锁检测，Innodb将session B杀掉。
